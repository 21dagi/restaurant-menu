...
import { Search, X, Star, ShoppingBag, Coffee, Flame, Droplets, Sparkles } from "lucide-react";
...
export default function MenuSection({ onCartChange, currencySymbol = "Br.", whatsappNumber, tableNumber }: MenuSectionProps) {
  ...
  return (
    <>
      <section id="menu" className="bg-[#111111] px-4 py-20 sm:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="text-center">
            <p className="font-bold uppercase tracking-[0.3em] text-yellow-400">Our Fresh Menu</p>
            <h2 className="mt-3 text-3xl font-black sm:text-4xl md:text-5xl">Burgers, Bites & Drinks</h2>
            <p className="mx-auto mt-3 max-w-2xl text-sm text-gray-400 sm:text-base">
              Hand-pressed smash burgers, wood-fired pizzas, crispy wings and freshly blended Ethiopian juices & shakes.
            </p>
          </div>

          {/* Search */}
          <div className="mx-auto mt-8 max-w-xl">
            <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-[#1b1b1b] px-5 py-3.5">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            <button
              ...
              className={`rounded-full px-4 py-2 text-xs font-bold transition sm:px-5 sm:py-2.5 sm:text-sm ${activeCategory === "all" ? "bg-yellow-400 text-black shadow-lg shadow-yellow-400/20" : "border border-white/10 bg-[#1b1b1b] text-gray-300 hover:border-yellow-400 hover:text-yellow-400"}`}
            >
              🍽️ All Items
            </button>
            {categories.map((cat) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-xs font-bold transition sm:px-5 sm:py-2.5 sm:text-sm ${activeCategory === cat.id ? "bg-yellow-400 text-black shadow-lg shadow-yellow-400/20" : "border border-white/10 bg-[#1b1b1b] text-gray-300 hover:border-yellow-400 hover:text-yellow-400"}`}
              >
                {cat.icon} {cat.name}
              </button>
            ))}
          ...
          {isDrinksActive && (
            <motion.div
              ...
              className="mt-4 flex flex-wrap justify-center gap-1.5 rounded-2xl border border-yellow-400/20 bg-yellow-400/5 p-2.5"
            >
                ...
                >
                  <div>
                    <div className="relative h-32 overflow-hidden sm:h-44 md:h-48">
                      <Image src={dish.image || "/images/hero.jpg"} alt={dish.name} fill className="object-cover transition duration-500 group-hover:scale-105" />
                      {!dish.isAvailable && (
            ...
            >
              ...
              <div className="grid sm:grid-cols-2">
                <div className="relative min-h-[220px] sm:min-h-[360px]">
                  <Image src={selectedDish.image || "/images/hero.jpg"} alt={selectedDish.name} fill className="object-cover" />
